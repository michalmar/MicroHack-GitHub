...
class ActivityCosmosService:
        ...
        try:
            ...
            for activity_data in sample_activities:
                try:
                    ...
                    logger.info(f"Seeded activity: {activity_data['id']} - {activity_data['type']} ({activity_data['notes']})")
                except cosmos_exceptions.CosmosResourceExistsError:
                    # Activity already exists, skip
                    logger.info(f"Activity {activity_data['id']} already exists, skipping")
                    pass
    ...
    def get_activity(self, activity_id: str) -> Optional[Activity]:
        ...
        try:
            ...
            response = self.container.read_item(item=activity_id, partition_key=activity_id)
            ...
            logger.error(f"CosmosDB HTTP error retrieving activity {activity_id}: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error retrieving activity {activity_id}: {e}")
            raise
    ...
    def search_activities(self, filters: ActivitySearchFilters) -> List[Activity]:
        """
        ...
        try:
            ...
            if filters.from_timestamp:
                ...
                parameters.append({"name": "@from_timestamp", "value": filters.from_timestamp.isoformat()})

            if filters.to_timestamp:
                conditions.append("c.timestamp <= @to_timestamp")
                parameters.append({"name": "@to_timestamp", "value": filters.to_timestamp.isoformat()})
    ...
    def delete_activity(self, activity_id: str) -> bool:
        ...
        try:
            ...
            self.container.delete_item(item=activity_id, partition_key=activity_id)
            ...
            logger.error(f"CosmosDB HTTP error deleting activity {activity_id}: {e}")
            raise
        except Exception as e:
