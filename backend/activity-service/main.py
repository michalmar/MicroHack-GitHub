...
async def lifespan(app: FastAPI):
    ...
    logger.info("Using lazy initialization - CosmosDB will be connected on first request")
...
async def get_activities(
    ...
    type: Optional[str] = Query(None, description="Filter by activity type (feed|walk|play|vet|train)"),
    from_param: Optional[datetime] = Query(None, alias="from", description="Filter activities from this timestamp (ISO format)"),
    to: Optional[datetime] = Query(None, description="Filter activities to this timestamp (ISO format)"),
    ...
    try:
        ...
        if not any([petId, type, from_param, to]):
            ...
            activities = db_service.search_activities(filters)
